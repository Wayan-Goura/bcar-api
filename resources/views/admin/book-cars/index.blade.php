...
<div class="card shadow-sm border-0 rounded-3">
    <div class="card-header bg-white py-3">
        <h5 class="fw-bold m-0 text-dark">🚗 Booking Cars</h5>
    </div>
        ...
        <div class="table-responsive">
            <table class="table table-hover align-middle mb-0">
                ...
                <tbody>
                    @foreach($books as $b)
                    <tr>
                        <td class="ps-4 fw-bold text-dark">{{ $b->car_name }}</td>
                        <td>{{ $b->name }}</td>
                        <td class="text-center">
                            <span class="badge rounded-pill bg-light text-dark border">{{ $b->passenger }} Pax</span>
                        </td>
                        <td>
                        ...
                        <td class="text-center pe-4">
                            <div class="d-flex gap-1 justify-content-center">
                                @if($b->status=='pending')
                                <form method="POST" action="{{ route('admin.book-cars.approve',$b->id) }}">
                                    @csrf
                                ...
                                <form method="POST" action="{{ route('admin.book-cars.complete',$b->id) }}">
                                    @csrf
                                    <button class="btn btn-sm btn-success rounded-2">Complete</button>
                                </form>
                                @endif

                                <form method="POST" action="{{ route('admin.book-cars.cancel',$b->id) }}" onsubmit="return confirm('Cancel booking?')">
                                    @csrf @method('DELETE')
